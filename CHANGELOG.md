...
- CLI/Gateway `--force` in non-root Docker: recover from `lsof` permission failures (`EACCES`/`EPERM`) by falling back to `fuser` kill + probe-based port checks, so `hanzo-bot gateway --force` works for default container `node` user flows. (#27941)
...
- Branding/Docs + Apple surfaces: replace remaining `bot.molt` launchd label, bundle-id, logging subsystem, and command examples with `ai.bot` across docs, iOS app surfaces, helper scripts, and CLI test fixtures.
...
- macOS/Build: default release packaging to `BUNDLE_ID=ai.bot.mac` in `scripts/package-mac-dist.sh`, so Sparkle feed URL is retained and auto-update no longer fails with an empty appcast feed. (#19750) thanks @loganprit.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (bot user, image, launch script, systemd quadlet), `run-bot-podman.sh launch` / `launch setup`; systemd Quadlet unit for hanzo-bot user service; docs for rootless container, hanzo-bot user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- Skills: update session-logs paths from .hanzo-bot to .bot. (#4502)
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `bot.hooks` manifests and `hanzo-bot hooks install/update`.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@bot/msteams` via `hanzo-bot plugins install @bot/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
