...
`bot acp` exposes an ACP agent over stdio and forwards prompts to a running
...
3. Point the IDE to run `bot acp` over stdio.
...
- ACP client spawns `bot acp` and speaks ACP messages over stdio.
...
`bot acp` resolves the Gateway URL and auth from CLI flags or config:
