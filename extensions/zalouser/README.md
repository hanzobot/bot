...
Botbot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
botbot channels login --channel zalouser
...
```bash
botbot message send --channel zalouser --target <threadId> --message "Hello from Botbot!"
```
...
```bash
botbot channels login --channel zalouser              # Login via QR
botbot channels login --channel zalouser --account work
botbot channels status --probe
botbot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
botbot directory self --channel zalouser
botbot directory peers list --channel zalouser --query "name"
botbot directory groups list --channel zalouser --query "work"
botbot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
botbot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
botbot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `botbot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
botbot channels login --channel zalouser --account work
botbot message send --channel zalouser --account work --target <id> --message "Hello"
