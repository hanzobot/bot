...
`bot.invoke --each`) without adding new Botbot code per workflow.
...
This extension depends on Botbot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Botbot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.botbot/extensions` as a standalone plugin directory.
