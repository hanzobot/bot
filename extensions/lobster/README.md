...
- This plugin integrates Lobster with Botbot *without core changes*.
...
## Using `bot.invoke` (Lobster → Botbot tools)

Some Lobster pipelines may include a `bot.invoke` step to call back into Botbot tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the Botbot Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- Botbot provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, Botbot returns `404 Tool not available`.
