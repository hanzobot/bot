...
import type { BotPluginService, DiagnosticEventPayload } from "bot/plugin-sdk";
import { onDiagnosticEvent, registerLogTransport } from "bot/plugin-sdk";
    ...
    async start(ctx) {
      ...
      if (logsEnabled) {
        ...
        stopLogTransport = registerLogTransport((logObj) => {
          ...
          if (meta?.name) attributes["bot.logger"] = meta.name;
      ...
      const recordModelUsage = (evt: Extract<DiagnosticEventPayload, { type: "model.usage" }>) => {
        ...
        if (usage.input) tokensCounter.add(usage.input, { ...attrs, "bot.token": "input" });
        if (usage.output) tokensCounter.add(usage.output, { ...attrs, "bot.token": "output" });
        if (usage.cacheRead)
        ...
        if (usage.total) tokensCounter.add(usage.total, { ...attrs, "bot.token": "total" });
      ...
      const recordWebhookProcessed = (
        ...
        if (evt.chatId !== undefined) spanAttrs["bot.chatId"] = String(evt.chatId);
      ...
      const recordMessageProcessed = (
        ...
        if (!tracesEnabled) return;
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.sessionKey) spanAttrs["bot.sessionKey"] = evt.sessionKey;
        if (evt.sessionId) spanAttrs["bot.sessionId"] = evt.sessionId;
        if (evt.chatId !== undefined) spanAttrs["bot.chatId"] = String(evt.chatId);
        if (evt.messageId !== undefined) spanAttrs["bot.messageId"] = String(evt.messageId);
        if (evt.reason) spanAttrs["bot.reason"] = evt.reason;
      ...
      const recordSessionState = (
        ...
        if (evt.reason) attrs["bot.reason"] = evt.reason;
