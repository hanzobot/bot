...
metadata: {"botbot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
## Botbot Runtime Mapping

- **Task tool** in the upstream spec == Botbot `sessions_spawn`
- **File I/O** == Botbot `read`/`write`
- **Remote fetch** == Botbot `web_fetch` (or `exec` with curl when POST is required)
