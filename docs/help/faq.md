...
### I can't access docs.hanzo.bot SSL error What now
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.hanzoai/bot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.hanzo/bot/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Bot treats as `<workspace>/skills`.
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Bot workspace); Bot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.hanzo/bot/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
Bot reads an optional **JSON5** config from `$BOT_CONFIG_PATH` (default: `~/.hanzoai/bot.json`):
...
Bot may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `hanzo-bot models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
Profiles also suffix service names (`ai.bot.<profile>`; legacy `com.bot.*`, `hanzo-bot-gateway-<profile>.service`, `Bot Gateway (<profile>)`).
