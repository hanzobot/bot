...
By default, the CLI installs skills into `./skills` under your current working directory. If a Bot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Bot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.hanzo/bot/skills` or bundled skills, workspace skills take precedence.
