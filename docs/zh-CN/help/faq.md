...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 Bot 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-bot)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [如何在 Linux 上安装 Bot？](#how-do-i-install-bot-on-linux)
  - [如何在 VPS 上安装 Bot？](#how-do-i-install-bot-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 Bot 自行更新吗？](#can-i-ask-bot-to-update-itself)
  ...
  - [如果我买了 Mac mini 运行 Bot，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-bot-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 Bot 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-bot-instances)
  ...
  - [在专用机器上运行 Bot 有多重要？](#how-important-is-it-to-run-bot-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 Bot 吗？有什么要求？](#can-i-run-bot-in-a-vm-and-what-are-the-requirements)
- [什么是 Bot？](#what-is-bot)
  - [用一段话描述 Bot？](#what-is-bot-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [Bot 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-bot)
  - [Bot 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-bot-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [Bot 能否按计划或在后台持续运行任务？](#can-bot-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [Bot 使用的所有数据都保存在本地吗？](#is-all-data-used-with-bot-saved-locally)
  - [Bot 将数据存储在哪里？](#where-does-bot-store-its-data)
  ...
  - [如何完全卸载 Bot？](#how-do-i-completely-uninstall-bot)
...
- [配置基础](#config-basics)
  ...
  - [Bot 浏览器可以无头运行吗？](#can-the-bot-browser-run-headless)
...
- [远程 Gateway 网关与节点](#remote-gateways-nodes)
  ...
  - [两个 Bot 实例（本地 + VPS）可以互相通信吗？](#can-two-bot-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [Bot 如何加载环境变量？](#how-does-bot-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 Bot 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-bot-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 Bot 但保留安装？](#how-do-i-completely-reset-bot-but-keep-it-installed)
  ...
  - [为什么 Bot 不在群组中回复？](#why-doesnt-bot-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [Bot、Flawd 和 Krill 使用什么模型？](#what-do-bot-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [如何以远程模式运行 Bot（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-bot-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 Bot？](#i-closed-my-terminal-on-windows-how-do-i-restart-bot)
...
- [安全与访问控制](#security-and-access-control)
  - [将 Bot 暴露给入站私信安全吗？](#is-it-safe-to-expose-bot-to-inbound-dms)
...
可以。通过 `~/.hanzoai/bot.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.hanzo/bot/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，Bot 将其视为 `<workspace>/skills`。
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 Bot 工作区）；Bot 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.hanzo/bot/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
Bot 从 `$BOT_CONFIG_PATH`（默认：`~/.hanzoai/bot.json`）读取可选的 **JSON5** 配置：
...
`hanzo-bot models status` 报告的是 **shell 环境导入**是否启用。"Shell env: off"**不**意味着你的环境变量缺失——它只意味着 Bot 不会自动加载你的登录 shell。
...
如果某个配置文件处于短期**冷却**（速率限制/超时/认证失败）或较长的**禁用**状态（计费/额度不足），Bot 可能会临时跳过它。要检查这一点，运行 `hanzo-bot models status --json` 并查看 `auth.unusableProfiles`。调优：`auth.cooldowns.billingBackoffHours*`。
...
配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.bot.*`、`hanzo-bot-gateway-<profile>.service`、`Bot Gateway 网关 (<profile>)`）。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [Bot 助手设置](/start/bot)和 [Agent send](/tools/agent-send)。
