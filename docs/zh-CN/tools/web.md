...
**推荐：** 运行 `hanzo-bot configure --section web`。它将密钥存储在 `~/.hanzo/bot/bot.json` 的 `tools.web.search.apiKey` 下。

**环境变量替代方案：** 在 Gateway 网关进程环境中设置 `BRAVE_API_KEY`。对于 Gateway 网关安装，将其放在 `~/.hanzo/bot/.env`（或你的服务环境）中。参见[环境变量](/help/faq#how-does-bot-load-environment-variables)。
