...
此外，你可以通过 `~/.hanzo/bot/bot.json` 中的 `skills.load.extraDirs` 配置额外的 Skills 文件夹（最低优先级）。
...
默认情况下，`skills-hub` 安装到当前工作目录下的 `./skills`（或回退到配置的 Hanzo Bot 工作区）。Hanzo Bot 在下一个会话中将其识别为 `<workspace>/skills`。
