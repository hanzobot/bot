...
Bot Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你的正常 Chrome 窗口），而不是启动一个单独的 hanzo-bot 管理的 Chrome 配置文件。
...
- 重新运行 `hanzo-bot browser extension install` 以刷新 Bot 状态目录下的已安装文件。
...
CLI 有意**不**打印 `node_modules` 路径。始终先运行 `hanzo-bot browser extension install` 将扩展复制到 Bot 状态目录下的稳定位置。
