...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`bot.internal.`），**或**
  - 手动主机/端口（备选）。
...
Gateway 网关在 `local.` 上广播 `_bot-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`bot.internal.`）和 Tailscale 分割 DNS。
