...
Hanzo Bot.app 不再捆绑 Node/Bun 或 Gateway 网关运行时。macOS 应用期望有一个**外部**的 `bot` CLI 安装，不会将 Gateway 网关作为子进程启动，而是管理一个每用户的 launchd 服务来保持 Gateway 网关运行（或者如果已有本地 Gateway 网关正在运行，则连接到现有的）。

## 安装 CLI（本地模式必需）

你需要在 Mac 上安装 Node 22+，然后全局安装 `bot`：
...
- `ai.hanzo.bot.gateway`（或 `ai.hanzo.bot.<profile>`；旧版 `com.bot.*` 可能仍然存在）

Plist 位置（每用户）：

- `~/Library/LaunchAgents/ai.hanzo.bot.gateway.plist`
  （或 `~/Library/LaunchAgents/ai.hanzo.bot.<profile>.plist`）
