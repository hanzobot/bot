...
- 你仍然可以使用 [Gateway 网关健康](/gateway/health) 中的 CLI 流程（`hanzo-bot status`、`hanzo-bot status --deep`、`hanzo-bot health --json`），并在 `/tmp/bot/bot-*.log` 中跟踪 `web-heartbeat` / `web-reconnect`。
