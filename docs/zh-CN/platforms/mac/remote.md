...
2. 确保 `bot` 在非交互式 shell 的 PATH 中（如需要，请符号链接到 `/usr/local/bin` 或 `/opt/homebrew/bin`）。
...
2. 在 **Hanzo Bot runs** 下，选择 **Remote over SSH** 并设置：
   ...
   - **CLI path**（高级）：可运行的 `bot` 入口点/二进制文件的可选路径（广播时自动填充）。
...
- **exit 127 / not found**：`bot` 不在非登录 shell 的 PATH 中。将其添加到 `/etc/paths`、你的 shell rc，或符号链接到 `/usr/local/bin`/`/opt/homebrew/bin`。
...
通过带有 `bot` 和 `node.invoke` 的脚本为每个通知选择声音，例如：
