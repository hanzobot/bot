...
summary: 研究笔记：Botd 工作区的离线记忆系统（Markdown 作为数据源 + 派生索引）
title: 工作区记忆研究
x-i18n:
...
目标：Botd 风格的工作区（`agents.defaults.workspace`，默认 `~/.hanzo/bot/workspace`），其中"记忆"以每天一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（例如 `memory.md`、`SOUL.md`）的形式存储。
...
- `memory.md` 保持"小型 + 类似核心"：你希望 Botd 每次会话都能看到的内容。
