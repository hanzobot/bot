...
使用 Nix 运行 Bot 的推荐方式是通过 **[nix-bot](https://github.com/hanzoai/nix-bot)** — 一个开箱即用的 Home Manager 模块。
...
>
> nix-bot 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `BOT_NIX_MODE=1` 时（nix-bot 会自动设置）：
