...
Bot 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `hanzo-bot update`）→ 验证。
...
- 当你运行打包的 `bot` 二进制文件（[`hanzo-bot.mjs`](https://github.com/hanzoai/bot/blob/main/hanzo-bot.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
