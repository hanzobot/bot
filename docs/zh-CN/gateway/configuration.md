...
read_when:
  - 添加或修改配置字段时
summary: ~/.hanzoai/bot.json 的所有配置选项及示例
title: 配置
x-i18n:
...
Bot 从 `~/.hanzoai/bot.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，Bot 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.hanzo/bot/workspace`）。通常只在以下情况需要配置：
...
请使用 `config.patch` 或 `hanzo-bot config set`。请备份 `~/.hanzoai/bot.json`。
...
- 如需稳定路径，将 `logging.file` 设为 `/tmp/hanzoai/bot.log`。
- 控制台输出可通过以下方式单独调整：
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@bot", "hanzo-bot"] } }],
  },
}
```
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[bot]"`，否则为 `""`（无前缀）。使用 `"[bot]"` 时，如果路由的智能体设置了 `identity.name`，Bot 会改用 `[{identity.name}]`。
...
- `group:nodes`：`nodes`
- `group:bot`：所有内置 Bot 工具（不包含提供商插件）
...
`~/.hanzo/bot/agents/<agentId>/agent/models.json` 或在 Bot 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（Bot.app 菜单栏或 `hanzo-bot gateway`）启动。
...
- Bot.app 监视 `~/.hanzoai/bot.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.hanzoai/bot.json`（或 `BOT_CONFIG_PATH`）并自动应用更改。
...
- `~/.hanzoai/bot.json`（或 `BOT_CONFIG_PATH`）
