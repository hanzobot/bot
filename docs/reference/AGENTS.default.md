...
Bot uses a dedicated workspace directory for the agent. Default: `~/.hanzo/bot/workspace` (configurable via `agents.defaults.workspace`).
...
- For browser-driven verification, use `hanzo-bot browser` (tabs/status/screenshot) with the Bot-managed Chrome profile.
