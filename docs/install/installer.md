...
read_when:
  - You want to understand `bot.hanzo.ai/install.sh`
...
Bot ships two installer scripts (served from `bot.hanzo.ai`):

- `https://bot.hanzo.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://bot.hanzo.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
 - `https://bot.hanzo.ai/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://bot.hanzo.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://bot.hanzo.ai/install.ps1))) -?
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://bot.hanzo.ai/install.sh | bash
```
...
```bash
curl -fsSL https://bot.hanzo.ai/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://bot.hanzo.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://bot.hanzo.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\bot"
```
