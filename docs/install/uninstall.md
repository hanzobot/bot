...
Default label is `ai.bot.gateway` (or `ai.bot.<profile>`; legacy `com.bot.*` may still exist):
...
If you used a profile, replace the label and plist name with `ai.bot.<profile>`. Remove any legacy `com.bot.*` plists if present.
