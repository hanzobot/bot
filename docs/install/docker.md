...
See [`ClawDock` Helper README](https://github.com/hanzoai/bot/blob/main/scripts/shell-helpers/README.md) for details.
...
`hanzo-bot-gateway` and `bot-cli` by generating `docker-compose.extra.yml`.
...
- noVNC observer access is password-protected by default; Bot provides a short-lived observer token URL instead of sharing the raw password in the URL.
