...
- Publish as `bot/plugin-sdk` (or export from core under `bot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Hanzo BotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Hanzo BotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Hanzo BotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Hanzo BotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Hanzo BotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Hanzo BotConfig): string;
  };
};
```
...
- Introduce `bot/plugin-sdk`.
- Add `api.runtime` to `Hanzo BotPluginApi` with the surface above.
