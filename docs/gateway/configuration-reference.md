...
Mattermost ships as a plugin: `bot plugins install @bot/mattermost`.
...
Bot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.bot/agents/<agentId>/agent/models.json`.
...
- Loaded from `~/.bot/extensions`, `<workspace>/.bot/extensions`, plus `plugins.load.paths`.
