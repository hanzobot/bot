...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `hanzo-bot configure`). If you are running Bot via Podman using the dedicated `bot` user, the config lives at `~bot/.hanzoai/bot.json`.
