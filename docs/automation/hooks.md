...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Bot. See [Webhook Hooks](/automation/webhook) or use `hanzo-bot webhooks` for Gmail helper commands.
...
Each `bot.hooks` entry must stay inside the package directory after symlink
