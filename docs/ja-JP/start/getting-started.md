...
<Info>
最速のチャット方法：Control UIを開く（チャンネル設定は不要）。`hanzo-bot dashboard`を実行してブラウザでチャットするか、<Tooltip headline="Gatewayホスト" tip="Bot Gatewayサービスを実行しているマシン。">Gatewayホスト</Tooltip>で`http://127.0.0.1:18789/`を開きます。
...
<AccordionGroup>
  ...
  <Accordion title="テストメッセージを送信">
    ...
    ```bash
    hanzo-bot message send --target +15555550123 --message "Hello from Bot"
    ```
