...
5. Send a first DM and approve pairing (`bot pairing approve signal <CODE>`).
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `bot pairing approve signal <PAIRING_CODE>`.
...
bot pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/bot/bot-$(date +%Y-%m-%d).log" | tail -20
```
