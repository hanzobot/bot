...
```
    ...
    If Bot is already running as a background service, use `hanzo-bot gateway restart` instead.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep Bot current (`hanzo-bot update`) so the Discord voice receive recovery logic is present
