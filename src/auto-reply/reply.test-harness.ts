...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".hanzo/bot", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.BOT_STATE_DIR = path.join(home, ".hanzo/bot");
    process.env.BOT_AGENT_DIR = path.join(home, ".hanzo/bot", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".hanzo/bot", "agent");
