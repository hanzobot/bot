    ...
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B") {
        ...
        if (domain === WIDE_AREA_DISCOVERY_DOMAIN) {
          return {
            stdout: [
              `Add 2 3 ${WIDE_AREA_DISCOVERY_DOMAIN} _bot-gw._tcp. Tailnet Gateway`,
              "",
            ].join("\n"),
            stderr: "",
            code: 0,
    ...
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (cmd === "dig") {
        ...
        if (
          ...
          qtype === "PTR" &&
          qname === "_bot-gw._tcp.bot.internal"
        ) {
          return {
            stdout: `studio-gateway._bot-gw._tcp.bot.internal.\n`,
            stderr: "",
            code: 0,
            signal: null,
            killed: false,
        ...
        if (
          server === "100.123.224.76" &&
          qtype === "SRV" &&
          qname === "studio-gateway._bot-gw._tcp.bot.internal"
        ) {
          return {
            stdout: `0 0 18789 studio.bot.internal.\n`,
            stderr: "",
            code: 0,
            signal: null,
            killed: false,
