...
function candidateBinDirs(opts: EnsureBotPathOpts): string[] {
  ...
  try {
    ...
    const siblingBot = path.join(execDir, "bot");
    if (isExecutable(siblingBot)) candidates.push(execDir);
  } catch {
    // ignore
  }
  ...
  if (isExecutable(path.join(localBinDir, "bot"))) candidates.push(localBinDir);
...
export function ensureBotCliOnPath(opts: EnsureBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.BOT_PATH_BOOTSTRAPPED)) return;
