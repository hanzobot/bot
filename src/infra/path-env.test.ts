...
describe("ensureBotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling bot exists", async () => {
    ...
    try {
      ...
      try {
        ...
        if (originalFlag === undefined) delete process.env.BOT_PATH_BOOTSTRAPPED;
        else process.env.BOT_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
