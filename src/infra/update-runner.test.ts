...
describe("runGatewayUpdate", () => {
  ...
  function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "hanzo-bot.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "hanzo-bot.mjs")} doctor --non-interactive --fix`]:
        {
          stdout: "",
        },
    });
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g @hanzo/bot@latest --no-fund --no-audit --loglevel=error") {
  ...
  it("retries global npm update with --omit=optional when initial install fails", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === "npm i -g @hanzo/bot@latest --omit=optional --no-fund --no-audit --loglevel=error"
      ) {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "hanzo-bot", version: "2.0.0" }),
