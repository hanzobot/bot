...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (hanzo-bot -> moltbot)", async () => {
    const root = await makeTempRoot();
    const legacySymlink = path.join(root, ".hanzo-bot");
    ...
    const targetMarker = path.join(root, ".hanzo/bot", "marker.txt");
    expect(fs.readFileSync(targetMarker, "utf-8")).toBe("ok");
    expect(fs.readFileSync(path.join(root, ".moltbot", "marker.txt"), "utf-8")).toBe("ok");
    expect(fs.readFileSync(path.join(root, ".hanzo-bot", "marker.txt"), "utf-8")).toBe("ok");
  });
});
