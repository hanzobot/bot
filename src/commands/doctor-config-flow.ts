...
function stripUnknownConfigKeys(config: BotConfig): {
  ...
  const next = structuredClone(config) as BotConfig;
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let candidate = structuredClone(baseCfg) as BotConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("bot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  return { cfg, path: snapshot.path ?? CONFIG_PATH_BOT, shouldWriteConfig };
}
