...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".bot", "disable-launchagent");
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("BOT_GATEWAY_TOKEN");
  const envPassword = await getenv("BOT_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `BOT_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `BOT_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv BOT_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv BOT_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
