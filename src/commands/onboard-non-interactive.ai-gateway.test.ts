...
describe("onboard (non-interactive): Vercel AI Gateway", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bot-onboard-gateway-"));
    ...
    try {
      ...
      const { CONFIG_PATH_BOT } = await import("../config/config.js");
      const cfg = JSON.parse(await fs.readFile(CONFIG_PATH_BOT, "utf8")) as {
        auth?: {
