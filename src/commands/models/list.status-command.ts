...
import { CONFIG_PATH_BOT, loadConfig } from "../../config/config.js";
...
export async function modelsStatusCommand(
  ...
  runtime.log(
    `${label("Config")}${colorize(rich, theme.muted, ":")} ${colorize(rich, theme.info, shortenHomePath(CONFIG_PATH_BOT))}`,
  );
  runtime.log(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\` or \`${formatCliCommand("bot configure")}\`.`
