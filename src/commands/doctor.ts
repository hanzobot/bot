...
import { CONFIG_PATH_BOT, readConfigFileSnapshot, writeConfigFile } from "../config/config.js";
...
export async function doctorCommand(
  ...
  const configPath = configResult.path ?? CONFIG_PATH_BOT;
  if (!cfg.gateway?.mode) {
    const lines = [
  ...
  await noteStateIntegrity(cfg, prompter, configResult.path ?? CONFIG_PATH_BOT);
  ...
  if (shouldWriteConfig) {
    ...
    const backupPath = `${CONFIG_PATH_BOT}.bak`;
