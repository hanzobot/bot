...
homepage: https://docs.bot.hanzo.ai/hooks#command-logger
metadata:
  {
    "botbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.botbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.botbot/logs/commands.log ~/.botbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/botbot`:
   ```
   /home/username/.botbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.botbot/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.botbot/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.botbot/logs/commands.log | jq .
```
