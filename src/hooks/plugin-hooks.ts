...
function normalizePluginHookEntry(api: BotPluginApi, entry: HookEntry): HookEntry {
  return {
    ...
    bot: {
      ...entry.bot,
      hookKey: entry.bot?.hookKey ?? `${api.id}:${entry.hook.name}`,
      events: entry.bot?.events ?? [],
    },
  };
}
...
async function loadHookHandler(
  ...
  try {
    ...
    const exportName = entry.bot?.export ?? "default";
...
export async function registerPluginHooksFromDir(
  ...
  for (const entry of hooks) {
    ...
    const events = normalizedEntry.bot?.events ?? [];
    if (events.length === 0) {
