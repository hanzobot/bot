...
describe("external CLI credential sync", () => {
  it("skips codex-cli sync when credentials already exist in another openai-codex profile", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-codex-dedup-skip-"));
    try {
      await withTempHome(
        async (tempHome) => {
  ...
  it("creates codex-cli profile when credentials differ from existing openai-codex profiles", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-codex-dedup-create-"));
    try {
      await withTempHome(
        async (tempHome) => {
          const codexDir = path.join(tempHome, ".codex");
  ...
  it("removes codex-cli profile when it duplicates another openai-codex profile", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-codex-dedup-remove-"));
    try {
      await withTempHome(
        async (tempHome) => {
          const codexDir = path.join(tempHome, ".codex");
