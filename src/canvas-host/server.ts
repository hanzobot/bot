...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.botCanvasA2UIAction);
  const hasAndroid = () => !!(window.botCanvasA2UIAction && typeof window.botCanvasA2UIAction.postMessage === "function");
  ...
  window.addEventListener("bot:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    ...
    const ok = window.botSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BOT_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "bot", "canvas"));
