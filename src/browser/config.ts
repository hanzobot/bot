...
import {
  ...
  DEFAULT_BOT_BROWSER_CONTROL_URL,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) return DEFAULT_BOT_BROWSER_COLOR;
  const normalized = value.startsWith("#") ? value : `#${value}`;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_BOT_BROWSER_COLOR;
 ...
 *
 * Note: this is a Bot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for bot CDP).
...
export function resolveBrowserConfig(cfg: BrowserConfig | undefined): ResolvedBrowserConfig {
  ...
  const envControlUrl = process.env.BOT_BROWSER_CONTROL_URL?.trim();
  const controlToken = cfg?.controlToken?.trim() || undefined;
  const derivedControlPort = (() => {
    const raw = process.env.BOT_GATEWAY_PORT?.trim();
    if (!raw) return null;
  ...
  const controlInfo = parseHttpUrl(
    cfg?.controlUrl ?? envControlUrl ?? derivedControlUrl ?? DEFAULT_BOT_BROWSER_CONTROL_URL,
    "browser.controlUrl",
  );
