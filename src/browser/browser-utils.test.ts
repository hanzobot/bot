...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-hanzo-bot-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    try {
      ...
      expect(headers["x-hanzo-bot-relay-token"]).toBeTruthy();
      expect(headers["x-hanzo-bot-relay-token"]).not.toBe("test-gateway-token");
    } finally {
