...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.bot.hanzo.ai/gateway/health",
          "https://docs.bot.hanzo.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.bot.hanzo.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        `Get the tokenized link anytime: ${formatCliCommand("bot dashboard --no-open")}`,
      ].join("\n"),
      "Token",
    );
  ...
  await prompter.note(
    ["Back up your agent workspace.", "Docs: https://docs.bot.hanzo.ai/concepts/agent-workspace"].join(
      "\n",
    ),
    "Workspace backup",
  );
  ...
  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.bot.hanzo.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ? [
          ...
          "Docs: https://docs.bot.hanzo.ai/tools/web",
        ].join("\n")
      : [
  ...
  await prompter.note(
    'What now: https://bot.hanzo.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened with your token; keep that tab to control Bot."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the tokenized link above."
        : "Onboarding complete. Use the tokenized dashboard link above to control Bot.",
  );
}
