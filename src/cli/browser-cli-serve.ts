  ...
  browser
    ...
    .action(async (opts: { bind?: string; port?: string; token?: string }) => {
      ...
      if (!isLoopbackBindHost(host) && !authToken) {
        defaultRuntime.error(
          danger(
            `Refusing to bind browser control on ${host} without --token (or BOT_BROWSER_CONTROL_TOKEN, or browser.controlToken).`,
          ),
        );
        defaultRuntime.exit(1);
      }
          ...
          [
            ...
            ...(authToken
              ? [
                  ...
                  `export BOT_BROWSER_CONTROL_TOKEN=${JSON.stringify(authToken)}`,
                ]
              : []),
          ].join("\n"),
        ),
