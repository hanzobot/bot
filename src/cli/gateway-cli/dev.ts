...
import { CONFIG_PATH_BOT, writeConfigFile } from "../../config/config.js";
...
export async function ensureDevGatewayConfig(opts: { reset?: boolean }) {
  ...
  const configExists = fs.existsSync(CONFIG_PATH_BOT);
  ...
  defaultRuntime.log(`Dev config ready: ${shortenHomePath(CONFIG_PATH_BOT)}`);
