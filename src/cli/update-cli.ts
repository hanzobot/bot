...
const DEFAULT_GIT_DIR = path.join(os.homedir(), "bot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  return trimmed.startsWith("bot@") ? trimmed.slice("bot@".length) : trimmed;
}
...
async function isBotPackage(root: string): Promise<boolean> {
  try {
    const raw = await fs.readFile(path.join(root, "package.json"), "utf-8");
    const parsed = JSON.parse(raw) as { name?: string };
    return parsed?.name === "bot";
  } catch {
    return false;
  }
}
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `BOT_GIT_DIR points at a non-git directory: ${params.dir}. Set BOT_GIT_DIR to an empty folder or a bot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isBotPackage(params.dir))) {
    throw new Error(`BOT_GIT_DIR does not look like a bot checkout: ${params.dir}.`);
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (switchToPackage) {
    ...
    const updateStep = await runUpdateStep({
      name: "global update",
      argv: globalInstallArgs(manager, `bot@${tag}`),
      timeoutMs: timeoutMs ?? 20 * 60_000,
      progress,
    });
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Bot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("bot doctor")}\` and \`${formatCliCommand("bot gateway restart")}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted("Examples: `npm i -g bot@latest` or `pnpm add -g bot@latest`"),
      );
    }
    defaultRuntime.exit(0);
    return;
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${formatCliCommand("bot gateway restart")}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("bot doctor")}\`, then \`${formatCliCommand("bot gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("bot gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `BOT_GIT_DIR points at a non-git directory: ${gitDir}. Set BOT_GIT_DIR to an empty folder or a bot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Bot to the latest version")
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.bot.hanzo.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.bot.hanzo.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
