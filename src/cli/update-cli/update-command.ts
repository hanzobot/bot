...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun) {
    ...
    if (switchToGit) {
      ...
      actions.push(`Run global package manager update with spec @hanzo/bot@${tag}`);
    }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Bot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("hanzo-bot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("hanzo-bot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g @hanzo/bot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g @hanzo/bot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
