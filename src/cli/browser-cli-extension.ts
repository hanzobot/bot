...
import { STATE_DIR_BOT } from "../config/paths.js";
...
function installedExtensionRootDir() {
  return path.join(STATE_DIR_BOT, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts?: {
  ...
  const stateDir = opts?.stateDir ?? STATE_DIR_BOT;
      ...
      defaultRuntime.error(
        info(
          [
            ...
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.bot.hanzo.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
        ...
        defaultRuntime.error(
          danger(
            [
              ...
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.bot.hanzo.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
