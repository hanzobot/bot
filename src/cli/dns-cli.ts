...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.bot.hanzo.ai/cli/dns")}\n`,
    );

  dns
    .command("setup")
    .description("Set up CoreDNS to serve bot.internal for unicast DNS-SD (Wide-Area Bonjour)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): bot.internal"));
      ...
      const serverPath = path.join(confDir, "bot.internal.server");
