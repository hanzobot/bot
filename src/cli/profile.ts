...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.bot${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  const stateDir = env.BOT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.BOT_STATE_DIR?.trim()) env.BOT_STATE_DIR = stateDir;
