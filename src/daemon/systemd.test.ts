  ...
  it.each([
    {
      ...
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/hanzo-bot-gateway.service",
    },
    {
      ...
      expected: "/home/test/.config/systemd/user/hanzo-bot-gateway-jbphoenix.service",
    },
    {
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/hanzo-bot gateway start --name "My Bot"')).toEqual([
      "/usr/bin/bot",
      "gateway",
      "start",
      "--name",
...
describe("systemd service control", () => {
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "hanzo-bot-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "hanzo-bot-gateway-work.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
    const stdout = { write } as unknown as NodeJS.WritableStream;
