...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { BOT_STATE_DIR: channelSecurityStateDir },
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("hanzo-bot-sbx-browser-old\nhanzo-bot-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "hanzo-bot-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "hanzo-bot-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("hanzo-bot-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("hanzo-bot-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "hanzo-bot-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "hanzo-bot-sbx-browser-exposed") {
        return {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.hanzo/bot",
