  ...
  if (stateDirStat.ok) {
    ...
    if (isWorldWritable(bits)) {
      findings.push({
        ...
        detail: `${params.stateDir} mode=${formatOctal(bits)}; other users can write into your Bot state.`,
      ...
      findings.push({
        ...
        detail: `${params.stateDir} mode=${formatOctal(bits)}; group users can write into your Bot state.`,
        remediation: `chmod 700 ${params.stateDir}`,
      });
...
function collectGatewayConfigFindings(cfg: BotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectBrowserControlFindings(cfg: BotConfig): SecurityAuditFinding[] {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.controlUrl/browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("bot security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const envToken = process.env.BOT_BROWSER_CONTROL_TOKEN?.trim();
  ...
  if (!isLoopback) {
    if (!controlToken) {
      findings.push({
        ...
        detail: `browser.controlUrl is non-loopback (${resolved.controlUrl}) but no browser.controlToken (or BOT_BROWSER_CONTROL_TOKEN) is configured.`,
        remediation:
          "Set browser.controlToken (or export BOT_BROWSER_CONTROL_TOKEN) and prefer serving over Tailscale Serve or HTTPS reverse proxy.",
      });
    }
