...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("connect (req) handshake returns hello-ok payload", async () => {
      const { CONFIG_PATH_BOT, STATE_DIR_BOT } = await import("../config/config.js");
      const ws = await openWs(port);
      ...
      expect(payload?.snapshot?.configPath).toBe(CONFIG_PATH_BOT);
      expect(payload?.snapshot?.stateDir).toBe(STATE_DIR_BOT);
