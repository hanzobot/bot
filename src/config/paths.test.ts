...
describe("state + config path candidates", () => {
  ...
  it("prefers ~/.hanzo/bot when it exists and legacy dir is missing", async () => {
    await withTempRoot("bot-state-", async (root) => {
      const newDir = path.join(root, ".hanzo/bot");
  ...
  it("falls back to existing legacy state dir when ~/.hanzo/bot is missing", async () => {
    await withTempRoot("bot-state-legacy-", async (root) => {
      const legacyDir = path.join(root, ".hanzo-bot");
