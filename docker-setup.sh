...
ensure_control_ui_allowed_origins() {
  ...
  current_allowed_origins="$(
    docker compose "${COMPOSE_ARGS[@]}" run --rm bot-cli \
...
BOT_CONFIG_DIR="${BOT_CONFIG_DIR:-$HOME/.hanzo/bot}"
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${BOT_CONFIG_DIR}:/home/node/.hanzo/bot"
...
docker compose "${COMPOSE_ARGS[@]}" up -d hanzo-bot-gateway
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f hanzo-bot-gateway"
echo "  ${COMPOSE_HINT} exec hanzo-bot-gateway node dist/index.js health --token \"$BOT_GATEWAY_TOKEN\""
