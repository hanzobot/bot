...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<BotTalkPTTStopPayload, Never>?
    ...
    func beginPushToTalk() async throws -> BotTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
    ...
    func endPushToTalk() async -> BotTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> BotTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> BotTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = BotTalkPTTStopPayload(
                captureId: captureId,
