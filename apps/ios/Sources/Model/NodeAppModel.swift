...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.hanzo.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.hanzo.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.hanzo.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.hanzo.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([BotLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([BotScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([BotSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([BotChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([BotPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func locationMode() -> BotLocationMode {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to Bot."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to Bot."
                            }
                        }
