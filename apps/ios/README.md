...
This is an **alpha** iOS app that connects to an Hanzo Bot Gateway as a `role: node`.
...
3. On iOS: Hanzo Bot → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Hanzo Bot.xcodeproj -scheme Hanzo Bot -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/Hanzo BotKit` contains the shared transport/types used by the iOS app.
