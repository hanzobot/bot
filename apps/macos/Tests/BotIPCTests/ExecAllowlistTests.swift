...
struct ExecAllowlistTests {
    ...
    @Test func resolveForAllowlistSplitsShellChains() {
        let command = ["/bin/sh", "-lc", "echo allowlisted && /usr/bin/touch /tmp/bot-allowlist-test"]
    ...
    @Test func resolveForAllowlistFailsClosedOnCommandSubstitution() {
        let command = ["/bin/sh", "-lc", "echo $(/usr/bin/touch /tmp/bot-allowlist-test-subst)"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func resolveForAllowlistFailsClosedOnQuotedCommandSubstitution() {
        let command = ["/bin/sh", "-lc", "echo \"ok $(/usr/bin/touch /tmp/bot-allowlist-test-quoted-subst)\""]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func resolveForAllowlistUnwrapsEnvShellWrapperChains() {
        let command = ["/usr/bin/env", "/bin/sh", "-lc", "echo allowlisted && /usr/bin/touch /tmp/bot-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: nil,
            cwd: nil,
