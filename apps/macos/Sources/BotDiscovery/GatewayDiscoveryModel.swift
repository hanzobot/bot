...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "com.bot", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in BotBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "com.bot.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        let domain = BotBonjour.wideAreaGatewayServiceDomain
    ...
    private func updateGateways(for domain: String) {
        ...
        if domain == BotBonjour.wideAreaGatewayServiceDomain,
