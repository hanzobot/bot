...
enum CommandResolver {
    ...
    private static func botManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".bot")
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/bot.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyBotInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let botPath = self.projectBotExecutable(projectRoot: root) {
            ...
            let missingEntry = """
            bot entrypoint missing (looked for dist/index.js or bin/bot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to botCommand; keep it as node alias.
