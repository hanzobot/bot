...
private final class TestChatTransport: @unchecked Sendable, BotChatTransport {
        ...
        attachments _: [BotChatAttachmentPayload]) async throws -> BotChatSendResponse
    {
        ...
        return BotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> BotChatSessionsListResponse {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { BotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
