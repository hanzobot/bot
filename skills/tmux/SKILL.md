...
```bash
SOCKET_DIR="${BOT_TMUX_SOCKET_DIR:-${BOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/hanzo-bot-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hanzo-bot.sock"
SESSION=hanzo-bot-python
...
- Use `BOT_TMUX_SOCKET_DIR` (legacy `BOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$BOT_TMUX_SOCKET_DIR/hanzo-bot.sock"`.
