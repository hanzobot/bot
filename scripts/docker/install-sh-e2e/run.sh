...
const versions = JSON.parse(execSync("npm view bot versions --json", { encoding: "utf8" }));
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  BOT_BETA=1 curl -fsSL "$INSTALL_URL" | bash
...
run_profile() {
  ...
  for _ in $(seq 1 60); do
    if bot --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  bot --profile "$profile" health --timeout 10000 --json >/dev/null
