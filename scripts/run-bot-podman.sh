...
# --- Container run (script in hanzo-bot home, run as bot) ---
EFFECTIVE_HOME="${HOME:-}"
...
CONFIG_DIR="${BOT_CONFIG_DIR:-$EFFECTIVE_HOME/.hanzo/bot}"
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$CONFIG_DIR:/home/node/.hanzo/bot:rw" \
    -v "$WORKSPACE_DIR:/home/node/.hanzo/bot/workspace:rw" \
    "${ENV_FILE_ARGS[@]}" \
    "$BOT_IMAGE" \
