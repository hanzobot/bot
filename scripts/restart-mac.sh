...
APP_PROCESS_PATTERN="Bot.app/Contents/MacOS/Bot"
...
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.bot.mac.plist"
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.bot.mac 2>/dev/null || true
}
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Bot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Bot.app" ]]; then
    APP_BUNDLE="/Applications/Bot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Bot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Bot.app"
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/com.bot.gateway.plist' | head -n 40 || true"
fi
