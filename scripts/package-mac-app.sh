...
APP_ROOT="$ROOT_DIR/dist/Bot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Bot"
BUNDLE_ID="${BUNDLE_ID:-com.bot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/bot/bot/main/appcast.xml}"
AUTO_CHECKS=true
...
/usr/libexec/PlistBuddy -c "Set :BotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :BotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
BOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/BotKit_BotKit.bundle"
if [ -d "$BOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/BotKit_BotKit.bundle"
  cp -R "$BOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/BotKit_BotKit.bundle"
else
  echo "WARN: BotKit resource bundle not found at $BOTKIT_BUNDLE (continuing)" >&2
fi
