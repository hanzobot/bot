...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Bot")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Bot")
...
DMG_TEMP="$(mktemp -d /tmp/bot-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/bot-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/bot-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/bot-dmg-limits.txt
