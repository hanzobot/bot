...
#
# Suggested cron: */30 * * * * /home/admin/bot/scripts/auth-monitor.sh
#
...
send_notification() {
    ...
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            ...
            bot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Bot is down. Run: ssh l36 '~/bot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
