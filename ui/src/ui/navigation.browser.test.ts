...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/hanzo-bot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/hanzo-bot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/hanzo-bot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__BOT_CONTROL_UI_BASE_PATH__ = "/bot";
    const app = mountApp("/hanzo-bot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/bot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/hanzo-bot/sessions");
  });
